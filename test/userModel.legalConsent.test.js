...
test("createUser stores legal consent and age confirmation timestamps and versions", async () => {
  ...
  assert.equal(capturedParams.length, 18);
  ...
  // preferred_language, appended last so the consent positions above did not
  // have to move. A registration that carries no language stores NULL.
  assert.equal(capturedParams[17], null);
