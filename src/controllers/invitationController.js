  ...
  try {
    ...
    try {
      ...
      if (action === "accept") {
        ...
        if (parseInt(memberCount.rows[0].count) >= invitation.max_members) {
...
const getTeamsWhereUserCanInvite = async (req, res) => {
  try {
    ...
    // Filter out teams that are at capacity
    const availableTeams = teamsResult.rows
      .filter((team) => parseInt(team.current_members_count) < team.max_members)
      .map((team) => ({
        id: team.id,
        name: team.name,
