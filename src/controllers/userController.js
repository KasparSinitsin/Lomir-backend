...
const updateUser = async (req, res) => {
  try {
    ...
    const updateFields = [];
    const queryParams = [userId]; // User ID will be the last parameter
    let paramPosition = 1;

    // Add fields that exist in the request
    if (first_name !== undefined) {
      updateFields.push(`first_name = $${paramPosition++}`);
      queryParams.splice(paramPosition-1, 0, first_name);
    }
    if (last_name !== undefined) {
      updateFields.push(`last_name = $${paramPosition++}`);
      queryParams.splice(paramPosition-1, 0, last_name);
    }
    if (bio !== undefined) {
      updateFields.push(`bio = $${paramPosition++}`);
      queryParams.splice(paramPosition-1, 0, bio);
    }
    if (postal_code !== undefined) {
      updateFields.push(`postal_code = $${paramPosition++}`);
      queryParams.splice(paramPosition-1, 0, postal_code);
    }
    if (avatar_url !== undefined) {
      updateFields.push(`avatar_url = $${paramPosition++}`);
      queryParams.splice(paramPosition-1, 0, avatar_url);
    }
